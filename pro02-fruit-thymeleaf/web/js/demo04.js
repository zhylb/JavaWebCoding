...
function addFruit() {
    ...
    let fnum = document.getElementById("fnum");
        ...
        tr = fruitTbl.insertRow(fruitTbl.rows.length - 1);


        tdname = tr.insertCell();
        tdname.innerText = fname.value;


        tdprice = tr.insertCell();
        tdprice.innerText = price.value;

        tdfnum = tr.insertCell();
        tdfnum.innerText = fnum.value;

        a = tr.insertCell();
        a.innerText = parseInt(tdprice.innerText) * parseInt(tdfnum.innerText);

        b = tr.insertCell();
        b.innerHTML = "<img src=\"../imgs/del.jpg\"class=\"delImg\"/>"
...
function ckInput() {
    ...
    //48-57 96-105 8删除键 回车13
    console.log(kc);

    if(!(kc >= 48 && kc <= 57 || kc >= 96 && kc<= 105 || kc == 8 || kc == 13)){
